...
class BookDetails {
    constructor(bookID, title, isbn, pageCount, thumbnailUrl, longDescription, authors, category) {
      ...
      this.thumbnailUrl = thumbnailUrl; this.longDescription = longDescription; this.authors = authors;
...
function LoadCategoryArray(data){
    ...
    for(let b of data.BookDetails){
        bookList.push(new BookDetails(b.bookID, b.title, b.isbn, b.pageCount, 
                     b.thumbnailUrl, b.longDescription, b.authors, b.category))
    }
...
function loadSecondPage(data){
    ...
    `;
        ...
        var ID = JSON.parse(localStorage.getItem("rowID"))
         ...
         for(let [index, value] of JSON.parse(localStorage.getItem("bookList")).entries()){
